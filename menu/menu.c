...
int show_menu()
{
    ...
    {// centered
        ...
        mlx_string_put(data.mlx_ptr, data.mlx_win, g_menu.menu_w / 2 - ft_strlen(cam_move) * FONT_WIDTH / 3,90, rgb_to_int("255,255,255"), cam_move);
        mlx_string_put(data.mlx_ptr, data.mlx_win, g_menu.menu_w / 2 - ft_strlen(cam_rot) * FONT_WIDTH / 3,110, rgb_to_int("255,255,255"), cam_rot);
    }
    return (1);
}
...
{
    mlx_string_put(data.mlx_ptr, data.mlx_win, g_resolution.x - 200, 50,
        rgb_to_int("255,255,255"), (selected_object ? ft_strjoin("Selected Object = ",selected_object->type) : ""));
    return (1);
}